...
public class TreeCategory<TI>(TreeView treeView, string icon, string name, int id, ITreeRepository<TI> treeRepository)
  ...
  protected ITreeRepository<TI> _treeRepository = treeRepository;

  public event EventHandler<TreeItemDroppedEventArgs>? AfterDropEvent;

  public override async Task ItemCreate(ITreeItem parent) {
    var newName = await GetNewName(true, string.Empty, parent, _treeRepository.ValidateNewItemName, Icon!);
    if (string.IsNullOrEmpty(newName)) return;

    try {
      parent.IsExpanded = true;
      var item = _treeRepository.ItemCreate(parent, newName);
  ...
  public override async Task ItemRename(ITreeItem item) {
    var newName = await GetNewName(true, item.Name, item, _treeRepository.ValidateNewItemName, Icon!);
    if (string.IsNullOrEmpty(newName)) return;

    try {
      _treeRepository.ItemRename(item, newName);
    }
    catch (Exception ex) {
      Log.Error(ex);
    }
  ...
  public override async Task ItemDelete(ITreeItem item) {
    ...
    try {
      if (UseTreeDelete)
        _treeRepository.TreeItemDelete(item);
      else
        _treeRepository.ItemDelete(item);
  ...
  public override async Task GroupMoveInItems(ITreeGroup group) {
    ...
    foreach (var item in itemsToMove)
      _treeRepository.ItemMove(item, group, false);
  }
...
public class TreeCategory<TI, TG>(TreeView treeView, string icon, string name, int id, ITreeRepository<TI> tr, ITreeRepository<TG> gtr)
  : TreeCategory<TI>(treeView, icon, name, id, tr) where TI : class, ITreeItem where TG : class, ITreeItem {

  protected ITreeRepository<TG> _groupTreeRepository = gtr;

  public override async Task GroupCreate(ITreeItem parent) {
    var newName = await GetNewName(false, string.Empty, parent, _groupTreeRepository.ValidateNewItemName, Icon!);
    if (string.IsNullOrEmpty(newName)) return;

    _groupTreeRepository.ItemCreate(parent, newName);
  }

  public override async Task GroupRename(ITreeGroup group) {
    var newName = await GetNewName(false, group.Name, group, _groupTreeRepository.ValidateNewItemName, Icon!);
    if (string.IsNullOrEmpty(newName)) return;

    _groupTreeRepository.ItemRename(group, newName);
  }

  public override void GroupMove(ITreeGroup group, ITreeGroup dest, bool aboveDest) =>
    _groupTreeRepository.ItemMove(group, dest, aboveDest);

  public override async Task GroupDelete(ITreeGroup group) {
    if (!await _deleteAccepted(group.Name)) return;

    _groupTreeRepository.ItemDelete(group);
  }
}